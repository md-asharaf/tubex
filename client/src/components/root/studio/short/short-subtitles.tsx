...
export const ShortSubtitles = () => {
    ...
    const { data: subtitleContent, isLoading: isSubtitleLoading } = useQuery({
        queryKey: ["subtitle", short?.subtitle],
        queryFn: async (): Promise<string> => {
            if (!short?.subtitle) return "No subtitle URL found.";
            try {
                const response = await fetch(short.subtitle);
        ...
        enabled: !!short && short.subtitleStatus === "READY" && !!short.subtitle,
    });
        ...
        <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
            <div>
                ...
                <p className="text-muted-foreground">
                    View generated subtitles for "{short.title}"
                </p>
            </div>
            ...
            <div className="bg-white dark:bg-black border rounded-lg p-4 sm:p-6 space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm">
                        Status: <span className={`font-semibold ${short.subtitleStatus === "READY" ? "text-green-500" : "text-yellow-500"}`}>{short.subtitleStatus}</span>
                    </div>
                </div>

                {short.subtitleStatus === "READY" ? (
                    isSubtitleLoading ? (
